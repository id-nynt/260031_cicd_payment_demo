...
Telemetry-triggered production rollback now has a bounded final reconsideration window configured by `rollback_reconsideration` in `config/controller_policy.yaml`. The agent records `rollback_selected`, suspends delivery and requests fresh deployment-correlated observations through Java percepts. Two consecutive healthy samples cancel the pending rollback and resume `master_goal`; expiration commits verified rollback. It cannot cancel a rollback already dispatched. The conventional controller uses the same final recheck budget. Guide 07 offers a four-trial focused follow-up covering stopped-candidate repair and rollback reconsideration.
...
Current paired timing: temporary errors last 35 seconds; observations begin after a shared 15-second pause; error and latency use a 30-second rolling window. Observation/repair budgets and healthy-sample requirements are unchanged. Publish a new control revision and begin a new study after changing timing.
