...
public final class ControllerEnvironment extends Environment {
    ...
    private boolean observeTelemetry(Structure action) throws Exception {
        ...
        if (scenario != null && !scenario.isBlank()) {
            ...
            else if (scenario.equals("telemetry_unknown")) { decision = "unknown"; reason = "scenario_unknown"; }
            else {
                if (scenario.equals("telemetry_delayed")) {
                    journal.event("telemetry_observation", null, Map.of("entity", entity, "round", 1,
            ...
            ProjectTelemetryProvider.Assessment assessment = null;
            for (int round = 1; round <= controller.observationAttempts(); round++) {
                assessment = new ProjectTelemetryProvider(telemetryProject, environment, entity,
                ...
                if (!assessment.decision().equals("unknown")) break;
                if (round < controller.observationAttempts()) {
                    Thread.sleep(controller.observationIntervalSeconds() * 1000L);
                }
            }
        ...
        addPercept(Literal.parseLiteral("telemetry(" + entity + "," + decision + ")"));
        informAgsEnvironmentChanged();
        return true;
    }
