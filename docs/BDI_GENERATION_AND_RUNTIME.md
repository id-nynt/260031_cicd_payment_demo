...
Both approaches use the 11 cases in `scripts/experiment-scenarios.json`, the saved workflow contract, the same worker revision, release sources and traffic profiles. `run_experiment.py --mechanism bdi` coordinates one BDI trial. Native workflow dispatch coordinates one conventional trial. See the [comparison guide](BDI_COMPARATIVE_EXECUTION_GUIDE.md) and [conventional manual](CONVENTIONAL_MANUAL_EXECUTION_GUIDE.md) for preparation, expected outcomes and measurement limitations.
...
Persistent generation is unchanged. New worker fault controls require publishing a new workflow revision, not regenerating the agent. Regenerate only when project inputs or generator sources change; startup validates existing artifacts.
