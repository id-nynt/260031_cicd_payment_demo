...
> Historical pre-canonical gate experiment. For current commands and verified recovery, use the [manual experiment walkthrough](../../execution/guidelines/03_BDI_MANUAL_EXECUTION_GUIDE.md). The description below records the earlier architecture.
...
4. Review all changes with `git status` and `git diff`. The copied `bdi-cicd-framework/` is currently untracked; it **must be committed** or the workflow checkout will not contain the gate. Do not accidentally stage unrelated edits in `docs/resources-and-plans/01_EXPERIMENT_PLAN.md`.
...
This workspace is currently on `experiment-v2`, not `main`. One safe way to publish only this experiment (leaving the unrelated local edit to `docs/resources-and-plans/01_EXPERIMENT_PLAN.md` alone) is:
...
Review the staged framework files before committing; `bdi/bin/`, build output, and Gradle caches are ignored. Open and merge a pull request into `main` through your normal policy. The merge's push runs the healthy path automatically; then trigger the high-error run below. Do not run `git add .` if you want to leave `docs/resources-and-plans/01_EXPERIMENT_PLAN.md` out of the commit.
