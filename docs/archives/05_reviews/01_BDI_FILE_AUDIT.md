> Historical file inventory. Superseded operating guides are now in `docs/archives/02_pre-policy-refactor`; see the current manual and setup reference for execution.
...
| Location | Classification / treatment |
|---|---|
| [docs/archives/01_legacy/pre-canonical/](../01_legacy/pre-canonical) | Intentional archive: old agents, `gate.mas2j`, `project.mas2j`, alternate Gradle files and manual rollback workflow. Preserve for the research comparison; not executable configuration. |
| [docs/archives/06_experiment-records/](../06_experiment-records) | Retained historical and current verification evidence. Preserve. |
| [BDI_EXPERIMENT.md](../02_pre-policy-refactor/03_BDI_EXPERIMENT.md), [BDI_ARCHITECTURE_AUDIT_AND_PLAN.md](../02_pre-policy-refactor/01_BDI_ARCHITECTURE_AUDIT_AND_PLAN.md), [BDI_CONTROLLER_EXPERIMENT_RESULTS.md](../02_pre-policy-refactor/02_BDI_CONTROLLER_EXPERIMENT_RESULTS.md) | Earlier experiment/audit/results material. Useful history, not current operating instructions. The architecture audit's opening update still describes launcher generation and should be read as historical. |
