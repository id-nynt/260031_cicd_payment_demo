...
| 8. `production-temporary` | Mixed request errors for 75 seconds, then normal traffic; decisions depend on changing observations | Reobserve, accept only after recovery; no restart of a running app; actual error/healthy phases |
...
Normal policy: one eligible transient/timeout retry, five-second retry delay, 60-second deployment pause, at most 36 observations within 180 seconds, five-second observation interval, two consecutive healthy samples, error rate ≤5%, p95 ≤500 ms, ready/available and fresh correlated telemetry. Repair policy: one production restart, 120 seconds of shared normal payment probes, and a 300-second decision budget starting at diagnosis. Queue waits count; in-flight calls can add bounded wall-clock overhead. The two stopped-candidate cases use these probes instead of the normal **production** traffic client; normal staging traffic remains. A failed restart intentionally has no successful repair probes.
