...
Follow [the manual walkthrough](../../BDI_MANUAL_EXECUTION_GUIDE.md): verify the dispatchable worker revision and Linux runner, configure repository/token/release SHA and reachable endpoints, establish a telemetry-verified baseline, retain its receipt, then run a compatible candidate and an authorized recovery experiment. Verify GitHub run identity, production health and `stopped/restored` without candidate achievement. Reconcile uncertain dispatch before another campaign.
