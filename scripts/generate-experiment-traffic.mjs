...
const modes = ['normal', 'high_latency', 'high_error_rate', 'unhealthy', 'request_faults', 'inject_error'];
...
if (mode === 'inject_error' && (await health.json()).experimentMode !== 'request_faults') {
  throw new Error('Deploy the candidate with experiment_mode=request_faults before injecting error traffic');
}
...
if (mode === 'unhealthy') {
  ...
  for (let index = 0; index < count; index++) {
    ...
    const response = await fetch(`${baseUrl}/payments`, {
      ...
      headers: { 'Content-Type': 'application/json', 'Idempotency-Key': randomUUID(),
        ...(mode === 'inject_error' ? { 'X-Experiment-Fault': 'error' } : {}) },
      body: JSON.stringify({
        amount: 5000,
        currency: 'AUD',
    ...
    else if (['high_error_rate', 'inject_error'].includes(mode) && response.status === 503 && body.error === 'experiment_injected_failure') injectedFailures++;
  ...
  if (['high_error_rate', 'inject_error'].includes(mode) && injectedFailures === 0) throw new Error('Expected at least one injected HTTP 503');
  if (!['high_error_rate', 'inject_error'].includes(mode) && injectedFailures > 0) throw new Error('Unexpected injected HTTP 503');
  console.log(`Summary: ${succeeded} succeeded, ${injectedFailures} injected HTTP 503`);
  if (continuous) await new Promise(resolve => setTimeout(resolve, 1000));
  } while (continuous);
}
