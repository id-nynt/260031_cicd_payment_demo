...
CATALOG = json.loads((ROOT/'scripts/experiment-scenarios.json').read_text())
...
def configuration():
    doc, model, _, _ = validate(ROOT/'bdi-cicd-framework')
    return doc, model, conventional_policy(doc)
...
def prepare(directory):
    ...
    if baseline:
        ...
        good = known_good_sha(receipt, runtime_settings(doc), model, os.environ['GITHUB_REPOSITORY'])
